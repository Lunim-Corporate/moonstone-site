...
export default function PitchDeckDownloads() {
  ...
  useEffect(() => {
    async function fetchFiles() {
      try {
        const listRes = await fetch("/api/documents/available-files?category=pitch-deck");
        if (!listRes.ok) {
          ...
          fileList.map(async (file): Promise<FileEntry> => {
            try {
              const urlRes = await fetch("/api/documents/secure-url", {
                ...
                body: JSON.stringify({ fileId: file.id, category: "pitch-deck" }),
              });
  ...
  return (
    <div className="mt-8 rounded p-6" style={{ backgroundColor: "#1a1a1a" }}>
      <h2 className="text-xl mb-4 text-(--cta-color)">Pitch Deck</h2>
